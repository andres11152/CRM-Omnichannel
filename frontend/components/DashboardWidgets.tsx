...
const CardContainer: React.FC<{children: React.ReactNode, className?: string, onClick?: () => void}> = ({children, className = "", onClick}) => (
    <div 
        onClick={onClick}
        className={`bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 flex flex-col ${onClick ? 'cursor-pointer' : ''} ${className}`}
    >
        {children}
    </div>
);
...
export const ActionButton: React.FC<{onClick: () => void, icon: React.ReactNode, text: string}> = ({ onClick, icon, text }) => (
    <button onClick={onClick} className="w-full flex items-center gap-3 p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all group">
...
export const SystemStatusModule: React.FC<SystemStatusProps> = ({ data }) => {
    ...
    return (
        <CardContainer className="h-full">
            ...
            {/* Footer metrics */}
            <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-3 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-500 flex justify-between">
