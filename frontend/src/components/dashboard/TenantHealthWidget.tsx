...
export const TenantHealthWidget: React.FC<Props> = ({ tenants, loading }) => {
  ...
  const handleImpersonate = async (id: string, name: string) => {
    if (!confirm(t("dashboard.tenant_health.impersonate_confirm", { name, defaultValue: `¿Acceder al panel de ${name}?` }))) return;
    ...
    try {
      ...
      const userToLogin = {
        ...
        companyId: id,
        name: t("dashboard.tenant_health.impersonation_mode", "Modo Impersonación"),
        avatar: "",
      ...
      toast.error(t("dashboard.tenant_health.err_impersonate", "Error al iniciar impersonación"));
    }
  };
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-reply-border-dark">
            <th className="pb-2 font-medium">{t("dashboard.tenant_health.client", "Cliente")}</th>
            <th className="pb-2 font-medium">{t("dashboard.tenant_health.plan", "Plan")}</th>
            <th className="pb-2 font-medium">{t("dashboard.tenant_health.health_risk", "Salud / Riesgo")}</th>
            <th className="pb-2 font-medium text-right">MRR</th>
            <th className="pb-2 font-medium text-right">{t("widgets.action", "Acción")}</th>
          </tr>
        </thead>
            ...
            >
              ...
              <td className="py-3">
                <div className="group/tooltip relative flex items-center gap-2 cursor-help">
                  ...
                  <div className="w-20 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${getScoreColor(tenant.healthScore).split(" ")[0]}`}
                      style={{ width: `${tenant.healthScore}%` }}
                    />
                  </div>
                  <span
                    className={`text-xs font-bold ${getScoreColor(tenant.healthScore).split(" ")[1]}`}
                  >
                    {tenant.healthScore}
                  </span>
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-gray-900 text-white text-xs rounded p-2 opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">
                    <div className="font-bold mb-1 border-b border-gray-700 pb-1">
                      {t("dashboard.tenant_health.risk_factors", "Factores de Riesgo:")}
                    </div>
              ...
              <td className="py-3 text-right">
                <button
                  ...
                  title={t("dashboard.tenant_health.inspect_account", "Inspeccionar Cuenta")}
                >
