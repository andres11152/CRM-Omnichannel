...
describe("WhatsAppIdUtils (backend copy)", () => {
  ...
  describe("getTargetJid", () => {
    ...
    // [CHARACTERIZATION] Same quirk as whatsapp-service's copy (these two files
    // are near-identical, byte-for-byte in logic): the group-detection heuristic
    // (`length > 15`) counts ALL characters, not just digits, so a
    // human-formatted phone number (spaces/+) long enough gets misrouted to
    // "...@g.us". Callers must pass a pre-cleaned digit string.
  ...
  describe("isRealPhoneNumber", () => {
    ...
    it("rejects 15+ digit ids starting with 120 (group-JID leftover)", () => {
      expect(WhatsAppIdUtils.isRealPhoneNumber("120123456789012")).toBe(false);
    });
  });
  ...
  describe("isValidCrmPhone", () => {
    ...
    it("rejects an invalid-per-libphonenumber 13-digit LID look-alike", () => {
      // Not a recognized country/subscriber format for any real E.164 country code.
      expect(WhatsAppIdUtils.isValidCrmPhone("3939223269449")).toBe(false);
    });
  });
  ...
  describe("extractDisplayPhone", () => {
    // [CHARACTERIZATION — LATENT BUG, present in both repo copies] `phone` is
    // run through getPhoneNumber() unmodified, but getPhoneNumber() requires a
    // JID-shaped "user@domain" string. channelId/conversationChannelId are
    // deliberately suffixed with "@s.whatsapp.net" before that same call, but
    // `phone` is not — so a bare digit string (exactly what Contact.phone
    // holds in the DB, and exactly what backend/src/types/ticket.types.ts:211
    // passes here) is silently rejected. In practice this function always
    // falls through to channelId. Not fixed here — flagged for a maintainer.
