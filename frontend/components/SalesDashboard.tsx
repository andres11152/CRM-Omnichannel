        ...
        <div className="h-full flex flex-col bg-reply-bg dark:bg-reply-bg-dark overflow-y-auto">
            ...
            <div className="p-8 space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {/* LEADERBOARD */}
                    <div className="lg:col-span-2 bg-white dark:bg-[#202c33] p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                        <h3 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
                            <span>🏆</span> Ránking de Actividad (Este Mes)
                        </h3>
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={4} className="text-center py-4 text-gray-500">No hay datos suficientes</td></tr>
                                    ) : (
                                        stats?.leaderboard?.map((agent: any, index: number) => (
                                            <tr key={agent.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                                ...
                                                <td className="px-4 py-3 text-center font-mono">{agent.activities}</td>
                                                <td className="px-4 py-3 text-center font-mono">{agent.dealsWon}</td>
                                                <td className="px-4 py-3 text-right font-mono text-gray-500">
                                                    {agent.activities > 0 ? ((agent.dealsWon / agent.activities) * 100).toFixed(1) : 0}%
                                                </td>
                                            </tr>
                                        ))
                                    )}
                    ...
                    <div className="bg-white dark:bg-[#202c33] p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                        <h3 className="font-bold text-gray-800 dark:text-white mb-4">Salud del Pipeline</h3>
                        <div className="space-y-4">
                            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                                <h4 className="font-bold text-blue-800 dark:text-blue-300 text-sm mb-1">💡 Consejo de Venta</h4>
                                <p className="text-sm text-blue-700 dark:text-blue-400">
                                    Tu tasa de cierre del {stats?.conversionRate || 0}% está {stats?.conversionRate > 20 ? 'por encima' : 'por debajo'} del promedio de la industria (20%). 
                                    {stats?.conversionRate < 20 && " Intenta calificar mejor a los leads antes de convertirlos en oportunidades."}
                                </p>
                            </div>

                             <div className="flex items-center justify-between text-sm">
                                <span className="text-gray-500">Deals Estancados (&gt;30 días)</span>
                                <span className="font-bold text-red-500">0</span> {/* Placeholder for future metric */}
                            </div>
                            <div className="flex items-center justify-between text-sm">
                                <span className="text-gray-500">Tareas Vencidas</span>
                                <span className="font-bold text-amber-500">0</span> {/* Placeholder */}
                            </div>
...
const StatCard: React.FC<{title: string, value: string, subtitle?: string, icon: string, color: string, bg: string, isLoading?: boolean}> = ({title, value, subtitle, icon, color, bg, isLoading}) => (
       ...
       <div>
         {isLoading ? (
           <div className="animate-pulse h-8 bg-gray-200 dark:bg-gray-700 rounded w-1/2 mb-2"></div>
             ...
             <>
                <p className="text-3xl font-bold text-reply-text dark:text-reply-text-dark">{value}</p>
