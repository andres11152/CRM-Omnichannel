...
export const MarketingDashboard: React.FC = () => {
  // TABS & STATE
  const [activeSection, setActiveSection] = useState<'builder' | 'history' | 'templates'>('builder');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    jsonData.forEach((row: any) => {
      if (row[0]) {
        ...
        if (phone.length >= 7) { // Basic validation
             phones.push(phone);
        }
      }
    });
  ...
  const handleCreateTemplate = async () => {
    ...
    try {
        ...
        if (res.ok) {
            setNewTemplate({ name: '', content: '', category: 'MARKETING', subject: '', preheader: '' });
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-[#0b141a] transition-colors duration-200">
      ...
      <div className="px-8 py-4 bg-white dark:bg-[#202c33] border-b border-gray-200 dark:border-gray-700 flex justify-end items-center">
        <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
           <button onClick={() => setActiveSection('builder')} className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${activeSection === 'builder' ? 'bg-white dark:bg-[#202c33] shadow text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
             Crear Campaña
           </button>
           <button onClick={() => setActiveSection('templates')} className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${activeSection === 'templates' ? 'bg-white dark:bg-[#202c33] shadow text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
             Plantillas
           </button>
           <button onClick={() => setActiveSection('history')} className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${activeSection === 'history' ? 'bg-white dark:bg-[#202c33] shadow text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
             Historial
           </button>
        </div>
      </div>
      ...
      <div className="p-8 flex-1 overflow-hidden">
        {activeSection === 'builder' && (
           <div className="flex h-full gap-8">
              <div className="w-2/3 bg-white dark:bg-[#202c33] rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
                 {/* STEPS HEADER */}
                 <div className="flex border-b border-gray-200 dark:border-gray-700">
                    {[1, 2, 3].map(step => (
                       <div key={step} className={`flex-1 py-4 text-center text-sm font-bold border-b-2 transition-colors ${builderStep === step ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-400'}`}>
                          {step === 1 && "1. Contenido"}
                          {step === 2 && "2. Audiencia"}
                 ...
                 <div className="p-8 flex-1 overflow-y-auto">
                    {builderStep === 1 && (
                       <div className="space-y-6">
                          <div>
                             <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nombre de Campaña</label>
                             <input type="text" value={campaignName} onChange={e => setCampaignName(e.target.value)} className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 bg-gray-50 dark:bg-[#111b21] text-gray-900 dark:text-white" placeholder="Ej: Promo Verano" />
                          </div>

                          <div>
                             <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Canal</label>
                             <div className="flex gap-4">
                                <button onClick={() => setSelectedChannel('WHATSAPP')} className={`flex-1 py-3 rounded-lg border font-bold text-sm transition-all ${selectedChannel === 'WHATSAPP' ? 'bg-green-600 text-white border-green-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600'}`}>
                                   WhatsApp
                                </button>
                                <button onClick={() => setSelectedChannel('EMAIL')} className={`flex-1 py-3 rounded-lg border font-bold text-sm transition-all ${selectedChannel === 'EMAIL' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600'}`}>
                                   Email
                                </button>
                                <button onClick={() => setSelectedChannel('SMS')} className={`flex-1 py-3 rounded-lg border font-bold text-sm transition-all ${selectedChannel === 'SMS' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600'}`}>
                                   SMS
                                </button>
                             </div>
                          </div>
                          ...
                          {selectedChannel === 'EMAIL' && (
                              <div className="animate-fade-in-down">
                                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Asunto del Correo</label>
                                  <input type="text" value={emailSubject} onChange={e => setEmailSubject(e.target.value)} className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 bg-gray-50 dark:bg-[#111b21] text-gray-900 dark:text-white" placeholder="¡Oferta Especial para ti!" />
                              </div>
                          )}
                          <div>
                             <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Plantilla</label>
                             <select value={selectedTemplateId} onChange={e => handleTemplateChange(e.target.value)} className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 bg-gray-50 dark:bg-[#111b21] text-gray-900 dark:text-white">
                    ...
                    {builderStep === 2 && (
                       <div className="space-y-6">
                          <div>
                             <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">Etiquetas Destino</label>
                             <div className="flex flex-wrap gap-3">
                                {tags.map(tag => (
                                   <button key={tag.id} onClick={() => toggleTag(tag.id)} className={`px-4 py-2 rounded-full text-sm font-bold border transition-all ${selectedTags.includes(tag.id) ? `${tag.color} ring-2 ring-offset-1 ring-indigo-500` : 'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600'}`}>
                                      {tag.name}
                                   </button>
                                ))}
                             </div>
                          ...
                          <div className="p-6 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800 text-center">
                             <div className="text-4xl font-extrabold text-indigo-600 dark:text-indigo-400 mb-2">{audienceCount}</div>
                             <div className="text-sm font-bold text-gray-600 dark:text-gray-300">Contactos Estimados</div>
                          </div>
                           ...
                           <div className="pt-6 border-t border-gray-200 dark:border-gray-700">
                             <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">
                                ...
                                <span className="ml-2 text-xs font-normal text-gray-500">
                                   (Primera columna: Teléfono ej. 57310...)
                                </span>
                             </label>
                             <div className="flex gap-4 items-center">
                                <label className="cursor-pointer bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                   Seleccionar Archivo
                                ...
                                {excelFileName && (
                                   <div className="text-sm text-green-600 font-medium flex items-center gap-1">
                                      ...
                                      <span className="text-gray-500">({targetPhones.length} números encontrados)</span>
                                   </div>
                                )}
                             </div>
                           </div>
                    ...
                    {builderStep === 3 && (
                       <div className="space-y-8">
                          <div className="bg-gray-50 dark:bg-[#111b21] p-5 rounded-lg border border-gray-200 dark:border-gray-700">
                             <h4 className="font-bold text-gray-800 dark:text-white mb-4">🗓️ Programación</h4>
                             <div className="flex gap-4 mb-4">
                                <button onClick={() => setScheduleMode('now')} className={`flex-1 py-3 rounded-lg border font-bold text-sm transition-all ${scheduleMode === 'now' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600'}`}>
                                   Enviar Ahora
                                </button>
                                <button onClick={() => setScheduleMode('later')} className={`flex-1 py-3 rounded-lg border font-bold text-sm transition-all ${scheduleMode === 'later' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600'}`}>
                                   Programar
                                </button>
                             </div>
                             ...
                             {scheduleMode === 'later' && (
                                 <div className="animate-fade-in-down">
                                     <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Fecha y Hora de Envío</label>
                                     <input 
                                        type="datetime-local" 
                                        value={scheduledDate} 
                                        onChange={(e) => setScheduledDate(e.target.value)} 
                                        className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 bg-white dark:bg-[#111b21] text-gray-900 dark:text-white"
                                     />
                                     <p className="text-xs text-gray-400 mt-1">La campaña se iniciará automáticamente en la fecha seleccionada.</p>
                                 </div>
                             )}
                          </div>

                          <div className="bg-gray-50 dark:bg-[#111b21] p-5 rounded-lg border border-gray-200 dark:border-gray-700">
                             <h4 className="font-bold text-gray-800 dark:text-white mb-2">🛡️ Protección Anti-Spam</h4>
                             <div className="mb-6">
                                <div className="flex justify-between text-xs font-bold text-gray-600 dark:text-gray-300 mb-2">
                                   <span>Lento</span>
                                   <span className="text-indigo-600">{config.messagesPerMinute} msgs/min</span>
                                   <span>Rápido</span>
                                </div>
                                <input 
                                  type="range" min="1" max="120" step="1" 
                                  value={config.messagesPerMinute} 
                                  onChange={e => setConfig({...config, messagesPerMinute: Number(e.target.value)})}
                 ...
                 <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#2a3942] flex justify-between">
                    {builderStep > 1 ? <button onClick={() => setBuilderStep(builderStep - 1)} className="px-6 py-2 rounded-lg border border-gray-300 dark:border-gray-600 font-bold">Atrás</button> : <div></div>}
                    {builderStep < 3 ? (
                       <button onClick={() => setBuilderStep(builderStep + 1)} disabled={builderStep === 1 && (!selectedTemplateId || !campaignName)} className="px-8 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 text-white rounded-lg font-bold">Siguiente</button>
                    ) : (
                       <button onClick={handleLaunch} disabled={isSending || audienceCount === 0} className="px-8 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white rounded-lg font-bold">
                           {isSending ? 'Procesando...' : (editingCampaignId ? '💾 Guardar Cambios' : '🚀 Lanzar')}
                       </button>
                    )}
                 </div>
              </div>
              ...
              {/* PREVIEW PANEL */}
              <div className="w-1/3 flex flex-col items-center justify-center bg-[#e5ddd5] dark:bg-[#0b141a] rounded-xl border border-gray-200 dark:border-gray-700 p-8 relative overflow-hidden">
                 ...
                 <div className="z-10 w-full max-w-sm bg-white dark:bg-[#202c33] rounded-lg shadow-md p-3 relative">
                    ...
                    {selectedTemplateId ? (
                        ...
                        <div className="text-center text-gray-400 py-8 italic">Selecciona una plantilla para previsualizar</div>
                    )}
                    <div className="text-[10px] text-gray-400 text-right mt-1">Ahora</div>
                 </div>
              </div>
           </div>
        )}
        ...
        {activeSection === 'templates' && (
            <div className="h-full flex gap-8">
                {/* TEMPLATE LIST */}
                <div className="flex-1 bg-white dark:bg-[#202c33] rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
                    <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-[#111b21]">
                        <div>
                           <h3 className="font-bold text-xl text-gray-800 dark:text-gray-200 flex items-center gap-2">
                              🎨 Galería de Plantillas
                           </h3>
                           <p className="text-sm text-gray-500 dark:text-gray-400">Gestiona tus diseños de correo y mensajes.</p>
                        </div>
                        <button onClick={() => setIsCreatingTemplate(true)} className="px-4 py-2 bg-indigo-600 shadow-lg shadow-indigo-500/30 text-white font-bold rounded-lg hover:bg-indigo-700 transition-all transform hover:scale-105 flex items-center gap-2">
                    ...
                    {templates.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8">
                            ...
                            <p className="font-bold">No tienes plantillas guardadas</p>
                            <p className="text-sm">Crea una nueva para empezar tu campaña.</p>
                        </div>
                    ) : (
                        <div className="overflow-y-auto flex-1 p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {templates.map(t => {
                                const bodyContent = Array.isArray(t.components) ? (t.components.find((c: any) => c.type === 'BODY')?.text || '') : '';
                                const isHtml = bodyContent.includes('<div') || bodyContent.includes('<table') || bodyContent.includes('<html');
                                
                                return (
                                    <div key={t.id} className="group relative border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-xl transition-all bg-white dark:bg-[#111b21] flex flex-col h-[320px]">
                                        {/* Preview Area */}
                                        <div className="flex-1 bg-gray-100 dark:bg-[#0b141a] overflow-hidden relative">
                                            {isHtml ? (
                                                <div className="w-[200%] h-[200%] transform scale-50 origin-top-left pointer-events-none bg-white">
                                                   <div dangerouslySetInnerHTML={{ __html: bodyContent }} />
                                                </div>
                                            ) : (
                                                <div className="p-4 text-xs text-gray-600 dark:text-gray-300 whitespace-pre-wrap font-mono">
                                            ...
                                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-sm">
                                                ...
                                                }} className="px-4 py-2 bg-white text-gray-900 rounded-full font-bold text-xs hover:bg-gray-100 scale-90 group-hover:scale-100 transition-transform">
                                                    Usar
                                                </button>
                                                <button onClick={() => handleDeleteTemplate(t.id)} className="p-2 bg-red-500 text-white rounded-full hover:bg-red-600 scale-90 group-hover:scale-100 transition-transform">
                                        ...
                                        <div className="p-4 border-t border-gray-100 dark:border-gray-800">
                                            <div className="flex justify-between items-start mb-1">
                                                <h4 className="font-bold text-gray-800 dark:text-white truncate pr-2" title={t.name}>{t.name}</h4>
                                                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 dark:bg-gray-700 text-gray-500 uppercase tracking-wider">
                                                    {t.category}
                                                </span>
                                            </div>
                                            <p className="text-[10px] text-gray-400">
                                                {isHtml ? '📧 Diseño HTML' : '💬 Mensaje Texto'}
                                            </p>
                                        </div>
                                    </div>
                                );
                    ...
                    <div className="fixed inset-0 z-50 bg-[#f8f9fa] dark:bg-[#0b141a] flex flex-col animate-fade-in font-sans">
                        ...
                        <div className="h-16 px-6 bg-white dark:bg-[#202c33] border-b border-gray-200 dark:border-gray-700 flex justify-between items-center shadow-sm z-30 relative">
                            <div className="flex items-center gap-4">
                                <button onClick={() => setIsCreatingTemplate(false)} className="p-2 -ml-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 transition-colors">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-gray-400">
                                        <span className="uppercase tracking-wider font-semibold">{newTemplate.category}</span>
                                        <span>•</span>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1 border border-gray-200 dark:border-gray-700 mr-2">
                                     <button onClick={manualUndo} disabled={historyIndex <= 0} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 disabled:opacity-30 transition-colors" title="Deshacer">
                                         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                     </button>
                                     <button onClick={manualRedo} disabled={historyIndex >= historyStack.length - 1} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 disabled:opacity-30 transition-colors" title="Rehacer">
                                ...
                                <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                                    <button onClick={() => setPreviewDevice('desktop')} className={`p-2 rounded-md transition-all ${previewDevice === 'desktop' ? 'bg-white dark:bg-gray-600 shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                    </button>
                                    <button onClick={() => setPreviewDevice('mobile')} className={`p-2 rounded-md transition-all ${previewDevice === 'mobile' ? 'bg-white dark:bg-gray-600 shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
                                ...
                                <button 
                                    onClick={() => toast.success('Prueba de envío simulada a ' + (localStorage.getItem('userEmail') || 'tu correo'))}
                            ...
                            {showEmailSettings && (
                                <div className="absolute top-16 right-20 w-80 bg-white dark:bg-[#202c33] shadow-xl rounded-xl p-6 z-50 border border-gray-200 dark:border-gray-700 animate-fade-in-down">
                                    <div className="flex justify-between items-center mb-4">
                                        ...
                                        <button onClick={() => setShowEmailSettings(false)} className="text-gray-400 hover:text-gray-600">✕</button>
                                    </div>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Subject Line (Asunto)</label>
                                            <input 
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Preheader (Texto Previo)</label>
                                            <textarea 
                                            ...
                                            />
                                            <p className="text-[10px] text-gray-400 mt-1">Este texto complementa el asunto para aumentar la tasa de apertura.</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="flex-1 flex overflow-hidden">
                            ...
                            <div className="w-[420px] flex flex-col border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-[#111b21] z-20 shadow-lg relative">
                                ...
                                <div className={`flex flex-col transition-all duration-500 ease-in-out ${showCodeEditor ? 'h-[45%]' : 'h-full'} overflow-hidden`}>
                                    ...
                                    <div className="p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#0b141a]/40 shrink-0">
                                        <div className="flex justify-between items-center mb-3">
                                            ...
                                            <div className="flex items-center gap-2 bg-gray-200 dark:bg-gray-800 rounded-full p-1 pl-3">
                                                ...
                                                >
                                                    <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${showCodeEditor ? 'translate-x-4.5 left-0.5' : 'left-0.5'}`}></div>
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div className="relative group">
                                            ...
                                            <textarea
                                                ...
                                                placeholder={newTemplate.content ? "Describe qué cambios quieres hacer..." : "Describe tu email ideal para empezar..."}
                                            ...
                                            >
                                                {isGeneratingAI ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"/> : <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-700">
                                        ...
                                        {isGeneratingAI && (
                                            <div className="flex items-center gap-3 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-xl animate-pulse">
                                                ...
                                                <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300 font-mono">{generationStatus}</span>
                                            </div>
                                        )}
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <button 
                                                    onClick={() => {
                                                        ...
                                                        setNewTemplate(prev => ({...prev, content: prev.content + footer}));
                                                ...
                                                >
                                                    <span className="flex items-center gap-2"><span>⚖️</span> Footer Legal Anti-Spam</span>
                                ...
                                <div className={`border-t border-gray-200 dark:border-gray-800 bg-[#1e1e1e] flex flex-col transition-all duration-500 ease-in-out ${showCodeEditor ? 'flex-1' : 'h-0 overflow-hidden'}`}>
                                    <div className="flex justify-between items-center px-4 py-2 bg-[#252526] border-b border-[#3e3e42] shrink-0">
                                        ...
                                        <div className="flex gap-2 text-[10px] text-gray-500 font-mono">
                                            <span>Ln {newTemplate.content?.split('\n').length || 0}</span>
                                            <span>UTF-8</span>
                                        </div>
                                    </div>
                                    <textarea 
                                ...
                                {newTemplate.content ? (
                                    ...
                                    >
                                        ...
                                        <iframe 
                                            srcDoc={
                                                previewDarkMode 
                                                ? newTemplate.content + `<style>body { background-color: #121212 !important; color: #e0e0e0 !important; } td { color: #e0e0e0 !important; } a { color: #8ab4f8 !important; } .wrapper { background-color: #1e1e1e !important; }</style>`
                                                : newTemplate.content
                                            } 
                            ...
                            <div className="absolute inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-8 animate-fade-in">
                                <div className="bg-white dark:bg-[#202c33] rounded-2xl shadow-2xl w-full max-w-4xl h-[80vh] overflow-hidden flex flex-col relative animate-scale-in">
                                    <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-[#111b21]">
                                        ...
                                        <button onClick={() => setShowMediaLibrary(false)} className="text-gray-500 hover:text-gray-700 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">✕</button>
                                    </div>
                                    <div className="flex-1 overflow-hidden relative">
                                        <MediaLibrary 
                                            onSelect={(media) => {
                                                const imgTag = `<img src="${media.url}" alt="Imagen" style="max-width: 100%; height: auto; border: 0; display: block;" />`;
                                                setNewTemplate(prev => ({ ...prev, content: prev.content + '\n' + imgTag }));
        ...
        {activeSection === 'history' && (
           <div className="bg-white dark:bg-[#202c33] rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 h-full overflow-hidden flex flex-col">
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                 <input type="text" placeholder="Buscar campaña..." value={historySearch} onChange={(e) => setHistorySearch(e.target.value)} className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[#111b21] text-sm dark:text-white" />
              </div>
                 ...
                 <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                       {filteredCampaigns.map(c => (
                           <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-[#2a3942] transition-colors">
                              ...
                              <td className="py-4 px-4 text-gray-600 dark:text-gray-300">
                                  {c.status === 'scheduled' && c.config?.scheduledAt ? (
                                      ...
                                      <div className="text-gray-500">
                                          {new Date(c.createdAt).toLocaleDateString()} <span className="text-xs">{new Date(c.createdAt).toLocaleTimeString()}</span>
                                      </div>
                                  )}
                              </td>
                              ...
                              <td className="py-4 px-4 text-right">
                                  <div className="flex justify-end gap-2">
                                      <button onClick={() => handleEditCampaign(c)} className="text-indigo-600 hover:text-indigo-800 font-bold text-xs">Editar</button>
                                      <button onClick={() => handleDeleteCampaign(c.id)} className="text-red-600 hover:text-red-800 font-bold text-xs">Eliminar</button>
                                  </div>
                              </td>
                           </tr>
                         ))}
