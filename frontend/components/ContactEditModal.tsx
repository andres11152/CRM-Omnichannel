...
export const ContactEditModal: React.FC<Props> = ({ isOpen, onClose, contact, onSuccess }) => {
  ...
  return createPortal(
    <div className={`${isDark ? 'dark' : ''} fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity`}>
      <div className="bg-white dark:bg-[#1f2937] rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up border border-gray-100 dark:border-gray-700">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto bg-white dark:bg-[#1f2937]">
          {/* Avatar */}
