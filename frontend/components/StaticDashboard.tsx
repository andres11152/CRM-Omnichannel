...
import { Ticket, Bot, Zap, MessageSquare, Mail, Users, Settings, Activity, Workflow } from 'lucide-react';
...
const OperationalLayout: React.FC<StaticDashboardProps> = ({ stats, activities, agentWorkloadData, onNavigate }) => {
    return (
        <div className="grid grid-cols-12 gap-6 p-1 pb-10">
            ...
            <div className="col-span-12 h-[600px]">
                 ...
                 <div className="h-full bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-800 shadow-sm rounded-xl overflow-hidden p-4">
                     <TicketsKanbanView />
                 </div>
            </div>
             ...
             <div className="col-span-12 lg:col-span-6 h-[350px] flex flex-col gap-4">
                 {/* Quick Actions Placeholder or extra stats */}
                  <div className="grid grid-cols-2 gap-4 h-full">
                       <ActionButton onClick={() => onNavigate && onNavigate('queue')} icon={<Mail className="w-5 h-5" />} text="Bandeja de Entrada" />
                       <ActionButton onClick={() => onNavigate && onNavigate('contacts')} icon={<Users className="w-5 h-5" />} text="Gestionar Contactos" />
                       <ActionButton onClick={() => onNavigate && onNavigate('flows')} icon={<Workflow className="w-5 h-5" />} text="Automatizaciones" />
                       <ActionButton onClick={() => onNavigate && onNavigate('settings')} icon={<Settings className="w-5 h-5" />} text="Configuración" />
                  </div>
            </div>
        </div>
    );
...
const StrategicLayout: React.FC<StaticDashboardProps> = ({ stats, salesFunnelData, topAgentsData, channelStatsData, plan }) => {
    return (
        <div className="grid grid-cols-12 gap-6 p-1 pb-10">
            ...
            <div className="col-span-12 lg:col-span-4 h-[400px]">
                <PlanUsageWidget planName={plan?.name || 'Sin Plan'} usage={plan?.usage || []} />
            </div>
        </div>
    );
};
...
const AnalyticalLayout: React.FC<StaticDashboardProps> = ({ stats, plan, activities, salesFunnelData, topAgentsData, channelStatsData, agentWorkloadData }) => {
    return (
         <div className="grid grid-cols-12 gap-6 p-1 pb-10">
            {/* Top Cards Row */}
            <div className="col-span-6 lg:col-span-3 h-[180px]">
                <StatCard title="Tickets Activos" value={stats.activeTickets} icon={<Ticket className="w-6 h-6" />} color="text-indigo-600 dark:text-indigo-400" bg="bg-indigo-50 dark:bg-indigo-900/20" />
            </div>
             <div className="col-span-6 lg:col-span-3 h-[180px]">
                <StatCard title="Resolución IA" value={stats.aiResolution} icon={<Bot className="w-6 h-6" />} color="text-purple-600 dark:text-purple-400" bg="bg-purple-50 dark:bg-purple-900/20" />
            </div>
             <div className="col-span-6 lg:col-span-3 h-[180px]">
                <StatCard title="Tiempo Respuesta" value={stats.avgResponseTime} icon={<Zap className="w-6 h-6" />} color="text-amber-600 dark:text-amber-400" bg="bg-amber-50 dark:bg-amber-900/20" />
            </div>
             <div className="col-span-6 lg:col-span-3 h-[180px]">
                <StatCard title="Total Mensajes" value={stats.totalMessages} icon={<MessageSquare className="w-6 h-6" />} color="text-blue-600 dark:text-blue-400" bg="bg-blue-50 dark:bg-blue-900/20" />
            </div>
...
export const StaticDashboard: React.FC<StaticDashboardProps & { currentLayout?: DashboardLayoutType }> = (props) => {
