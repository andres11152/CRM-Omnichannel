...
export const AgentWorkspace: React.FC<Props> = ({ aiConfig, user }) => {
  ...
  useEffect(() => {
    ...
    // Stop polling after 10s (socket should be connected by then)
    const stopPolling = setTimeout(() => clearInterval(checkInterval), 10000);
  ...
  //   - socketConnected = CRM real-time link (socket.io)
  //   - session.status  = WhatsApp API session (Baileys)
  useEffect(() => {
  ...
  // - Groups are NEVER shown in the queue tab (they go in the groups section of my_chats)
  ...
  if (activeTab === "my_chats") {
    ...
    if (isAdminRole) {
      // Pull ALL active group tickets from the full curated list
      ...
      groupContacts = allActiveGroupTickets.map((t) => {
        const displayName = resolveContactName(
          t.contact as Partial<Contact>,
          t.subject || undefined,
        );
        return {
          ...
          name: displayName,
          id: t.id,
          realContactId: t.contact.id,
          avatarUrl: t.contact.avatarUrl || "",
          lastMessage: t.lastMessage,
          lastMessageTime: new Date(t.lastMessageAt),
          unreadCount: t.unreadCount,
          tags: t.tags,
          status: t.status,
          profilePicUrl: t.contact.profilePicUrl || undefined,
          about: t.contact.about || undefined,
          assignedMode: (t.queueId === "ai" ? "bot" : "human") as
            | "bot"
            | "human",
          queueName: t.contact.queueName || t.queue?.name,
          assignedAgentName: t.contact.assignedAgentName || t.assignedTo?.name,
      ...
      // Agents: Only their assigned groups (already filtered in displayedTickets)
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 bg-white/80 dark:bg-reply-surface-dark/95 backdrop-blur-xl border-b border-gray-200/60 dark:border-reply-border-dark sticky top-0 z-40 transition-all duration-300 shadow-sm relative">
        <div className="flex justify-between items-center max-w-full gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            ...
            {/* Sync Button Removed by User Request - Consolidating to Chat Header Sync only */}
        ...
        >
          {activeTicket ? (
            activeTab === "my_chats" ? (
              ...
              // QUEUE PREVIEW VIEW (Pick Ticket) — Enterprise Design
                ...
                <div className="max-w-md w-full space-y-4">
                  ...
                  <div className="bg-white dark:bg-reply-panel-dark rounded-2xl shadow-xl border border-gray-200/80 dark:border-reply-border-dark overflow-hidden">
                        ...
                        <div className="flex items-center flex-wrap gap-1.5 mt-2">
                          ...
                          {activeTicket.priority && (
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold border
                              ${activeTicket.priority === "CRITICAL" || activeTicket.priority === "HIGH"
                              ...
                              {activeTicket.priority === "CRITICAL" ? "Crítico" : activeTicket.priority === "HIGH" ? "Alta" : activeTicket.priority === "MEDIUM" ? "Media" : "Baja"}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="px-6 py-4">
                      <div className="flex items-center gap-2 mb-2.5">
                        ...
                        <span className="ml-auto text-[10px] text-gray-400 dark:text-gray-500">
                          {(() => {
                            try {
                              return new Date(activeTicket.lastMessageAt).toLocaleString("es-CO", {
                                day: "numeric",
                                month: "short",
                                hour: "2-digit",
                                minute: "2-digit",
                    ...
                    <div className="px-6 pb-4 grid grid-cols-2 gap-2.5">
                      ...
                      <div className="bg-gray-50 dark:bg-[#111b21] rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-700/50">
                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Espera</p>
                        <p className="text-sm font-bold text-amber-600 dark:text-amber-400 mt-0.5">
                          {(() => {
                            const mins = Math.floor(
                              (Date.now() - new Date(activeTicket.createdAt).getTime()) / 60000,
                            );
                  ...
                  {/* CTA Button — Outside card for visual weight */}
                  {isRestricted ? (
