...
export const ApiDocsTab: React.FC = () => {
    ...
    () => [
      {
        method: "POST",
        path: "/messages/send",
        label: t("developer_settings.api_docs_tab.endpoints.send_message", "Enviar Mensaje WhatsApp"),
        color: "emerald",
        body: `{
  "to": "573001234567",
  "text": "Hola, este es un mensaje automático desde la API de Sentry."
      ...
      },
      {
        method: "POST",
        path: "/contacts",
        label: t("developer_settings.api_docs_tab.endpoints.upsert_contact", "Crear o Actualizar Contacto"),
        color: "emerald",
        body: `{
  "name": "Juan Pérez",
  "phone": "573001234567",
  ...
  "tags": ["Cliente VIP", "API"]
      ...
      },
      {
        method: "POST",
        path: "/deals",
        label: t("developer_settings.api_docs_tab.endpoints.create_deal", "Crear Oportunidad (Deal)"),
        color: "emerald",
        body: `{
  ...
  "stageId": "<cuid_stage>"
      ...
      },
      {
        method: "GET",
        path: "/tickets",
        label: t("developer_settings.api_docs_tab.endpoints.list_tickets", "Listar Tickets"),
        color: "sky",
        body: null,
      },
    ],
      ...
      <Card className="p-6">
        <div className="mb-6">
          <h3 className="font-bold text-reply-text-primary dark:text-white text-xl flex items-center gap-2">
            ...
            {t("developer_settings.api_docs_tab.title", "Referencia de API REST")}
          </h3>
          <p className="text-sm text-reply-text-secondary dark:text-reply-text-secondary-dark mt-2 leading-relaxed">
            {t("developer_settings.api_docs_tab.description_prefix", "Integra Sentry con tus sistemas internos. Autentícate enviando tu API Key en el header")}{" "}
            ...
            . {t("developer_settings.api_docs_tab.base_url_label", "Base URL:")}{" "}
            <code className="bg-reply-bg dark:bg-reply-bg-dark px-1.5 py-0.5 rounded text-reply-brand dark:text-reply-brand-light font-mono text-xs border border-reply-border dark:border-reply-border-dark">
        ...
        <div className="space-y-8">
          ...
          <div className="border border-amber-200 dark:border-amber-900/50 rounded-xl overflow-hidden shadow-sm">
            <div className="bg-amber-50 dark:bg-amber-950/30 p-4 border-b border-amber-200 dark:border-amber-900/50 flex items-center gap-4">
              ...
              <span className="font-mono text-sm text-reply-text-primary dark:text-white font-bold">{t("developer_settings.api_docs_tab.webhook_payload_title", "Estructura de Evento Webhook")}</span>
            </div>
