...
export const FlowBuilder: React.FC = () => {
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent) => {
    if (isDragging && draggedNodeId) {
      ...
      setFlow(prev => {
        ...
        return {
          ...prev,
          nodes: prev.nodes.map(n => n.id === draggedNodeId ? { ...n, position: { x, y } } : n)
        };
      });
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-gray-100 dark:bg-[#0b141a] overflow-hidden transition-colors duration-200">
      ...
      <ModuleHeader
        ...
        action={
          <div className="flex gap-3">
             ...
             <button onClick={handleSaveFlow} className="px-6 py-2 bg-white text-indigo-600 hover:bg-gray-100 rounded-lg text-sm font-bold shadow-md transition-colors">
               Guardar Flujo
             </button>
          </div>
        }
        ...
        {selectedNode ? (
          ...
          <div className="w-80 bg-white dark:bg-[#111b21] border-l border-gray-200 dark:border-gray-800 flex flex-col">
             <div className="p-4 border-b border-gray-200 dark:border-gray-800">
               <h3 className="font-bold text-gray-800 dark:text-white">Configuración del Flujo</h3>
             </div>
             <div className="p-4 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                   <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tipo de Disparador</label>
                   <select 
                ...
                {flow.triggerType === 'KEYWORD' && (
                    <div>
                        ...
                        <input 
                            ...
                            onChange={(e) => setFlow({
                                ...flow, 
                                triggerConfig: { ...flow.triggerConfig, keyword: e.target.value }
                            })}
                            className="w-full bg-gray-100 dark:bg-gray-800 border-none rounded p-2 text-sm text-gray-800 dark:text-white"
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <select 
                                ...
                                onChange={(e) => setFlow({
                                    ...flow, 
                                    triggerConfig: { ...flow.triggerConfig, event: e.target.value }
                                })}
                                className="w-full bg-gray-100 dark:bg-gray-800 border-none rounded p-2 text-sm text-gray-800 dark:text-white"
                            >
                                <option value="">Seleccionar Evento...</option>
                                <option value="DEAL_CREATED">Nuevo Deal Creado</option>
                                <option value="DEAL_UPDATED">Deal Actualizado (Cambio de Etapa)</option>
                            </select>
                        </div>
                        
                        {flow.triggerConfig?.event === 'DEAL_UPDATED' && (
                             <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Condición: Nueva Etapa</label>
                                <select 
                                    ...
                                    onChange={(e) => setFlow({
                                        ...
                                        triggerConfig: { 
                                            ...flow.triggerConfig, 
                                            condition: { ...flow.triggerConfig?.condition, stage: e.target.value }
                                        }
                                    })}
                                    className="w-full bg-gray-100 dark:bg-gray-800 border-none rounded p-2 text-sm text-gray-800 dark:text-white"
                                >
