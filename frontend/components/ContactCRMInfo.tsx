...
export const ContactCRMInfo: React.FC<Props> = ({ contact, onOpenTimeline }) => {
    ...
    const activeDeals = deals.filter(d => 
        d.stage?.name !== 'Ganado' && d.stage?.name !== 'Perdido'
    );
    ...
    return (
        <div className="bg-white dark:bg-[#202c33] border-b border-gray-200 dark:border-gray-700 px-4 py-3 space-y-3">
            {/* Contact Header */}
            ...
            <div className="space-y-2 text-xs">
                {contact.email && contact.email !== 'null@example.com' && !contact.email.includes('@whatsapp') && (
            ...
            <div className="pt-3 border-t border-gray-100 dark:border-gray-700">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button 
                        ...
                        className="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-2 py-1 rounded hover:bg-indigo-100 dark:hover:bg-indigo-800 transition-colors font-bold flex items-center gap-1"
                    >
                ...
                {loading ? (
                    ...
                    <div className="space-y-2">
                        <div className="grid grid-cols-2 gap-2 text-xs">
                            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-2 rounded border border-indigo-100 dark:border-indigo-800">
                                <div className="text-[10px] text-gray-500 dark:text-gray-400 mb-0.5">Oportunidades Activas</div>
                                <div className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{activeDeals.length}</div>
                            </div>
                            <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded border border-green-100 dark:border-green-800">
                                <div className="text-[10px] text-gray-500 dark:text-gray-400 mb-0.5">Negocios Cerrados</div>
                                <div className="text-sm font-bold text-green-600 dark:text-green-400">${(totalWonValue / 1000).toFixed(1)}K</div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            {activeDeals.slice(0, 3).map(deal => (
                                <div key={deal.id} className="bg-gray-50 dark:bg-gray-800/50 p-2 rounded text-xs group cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                    <div className="flex justify-between items-start gap-2 mb-1">
                                        <span className="font-medium text-gray-800 dark:text-gray-200 line-clamp-1 flex-1">{deal.title}</span>
                                        <span className="text-xs font-bold text-gray-600 dark:text-gray-400 whitespace-nowrap">${(deal.value / 1000).toFixed(1)}K</span>
                                    </div>
                    ...
                    <div className="text-xs text-gray-400 italic text-center py-4 border border-dashed border-gray-200 dark:border-gray-700 rounded bg-gray-50 dark:bg-gray-800/30">
                        No hay oportunidades activas
                        <button onClick={() => setShowDealModal(true)} className="block mx-auto mt-1 text-indigo-500 hover:text-indigo-600 font-medium">Crear la primera</button>
                    </div>
                )}
            </div>
