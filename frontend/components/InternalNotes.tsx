...
export const InternalNotes: React.FC<Props> = ({
  ...
  return (
    <>
      <div
        className={`flex flex-col h-full bg-yellow-50 dark:bg-[#1f2c33] border-l border-gray-200 dark:border-gray-700 flex-shrink-0 transition-colors ${className || "w-72"}`}
      >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          {loading ? (
            ...
            notes.map((note) => (
              <div
                key={note.id}
                className="bg-white dark:bg-[#111b21] p-3 rounded-lg shadow-sm border border-yellow-100 dark:border-gray-700 group relative"
              >
                {editingId === note.id ? (
                  <div>
                    <textarea
