...
export const MediaPicker: React.FC<MediaPickerProps> = ({
  ...
  const availableFilters = allowedTypes && allowedTypes.length > 0
    ? ['ALL', ...allowedTypes]
    : ['ALL', 'IMAGE', 'AUDIO', 'VIDEO', 'DOCUMENT'];

  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#2a3942] rounded-xl shadow-2xl max-w-5xl w-full max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-gray-400">
              ...
              <p className="text-sm mt-1">Sube archivos desde la Biblioteca Multimedia</p>
            </div>
          ) : (
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {media.map(item => (
                ...
                >
                  ...
                  {/* Info */}
                  <div className="p-2">
                    <p className="text-xs font-semibold text-gray-800 dark:text-white truncate" title={item.originalName}>
                      {item.originalName}
                    </p>
