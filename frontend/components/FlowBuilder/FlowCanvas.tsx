...
export const FlowCanvas: React.FC<FlowCanvasProps> = ({
  ...
  const getNodeColor = (type: NodeType) => {
    switch (type) {
      ...
      // Legacy (deprecated)
      case 'message': return 'bg-blue-600 border-blue-400';
      case 'input': return 'bg-orange-500 border-orange-300';

      case 'action_email': return 'bg-pink-600 border-pink-400';
      case 'action_task': return 'bg-indigo-600 border-indigo-400';
      case 'action_calendar': return 'bg-red-600 border-red-400';
  ...
  const getNodeIcon = (type: NodeType) => {
    switch (type) {
      ...
      case 'action_task': return '📝';
      case 'action_calendar': return '📅';
      
      default: return '📦'; // Icono de paquete en lugar de texto
    }
  };
  ...
  return (
    <div 
      className="flex-1 bg-[#f0f2f5] dark:bg-[#111b21] relative overflow-hidden cursor-grab active:cursor-grabbing"
      ref={canvasRef}
    ...
    >
       ...
       <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 overflow-visible">
         ...
         {edges.map(edge => {
           ...
           return (
             <g key={edge.id}>
                {/* Glow Filter Definition (only needs to be defined once, but okay here for now) */}
                <defs>
       ...
       {nodes.map(node => (
         <div
           ...
           className={`absolute w-[200px] bg-white dark:bg-[#202c33] rounded-lg shadow-md border-2 transition-all flow-node z-10 ${
              selectedNodeId === node.id ? 'ring-2 ring-indigo-500 border-transparent' : 'border-gray-200 dark:border-gray-600'
           }`}
         ...
         >
            ...
            <div className="p-3">
               ...
               <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 bg-gray-50 dark:bg-[#111b21] p-1.5 rounded">
