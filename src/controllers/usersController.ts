...
export const updateUser = catchAsync(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  if (id !== req.user?.id) {
    return next(new AppError('No tienes permiso para editar este perfil.', 403));
  }
...
export const deleteUser = catchAsync(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  if (id !== req.user?.id) {
    return next(new AppError('No tienes permiso para eliminar usuarios.', 403));
  }
  return next(new AppError('La eliminación de usuarios no está permitida a través de esta ruta.', 403));
  ...
  if (!user) {
    return next(new AppError('No se encontró un usuario con ese ID para eliminar', 404));
  }
