...
export class MessageRevocationHandler {
  ...
  async handleRevocation(
    ...
    if (!validated.success) {
      Logger.warn(`[RevocationHandler] ⚠️ Invalid revocation payload dropped`, {
        sessionId,
    ...
    if (!companyId) {
      Logger.warn(
        `[RevocationHandler] ⚠️ No companyId found for session ${sessionId}`,
      );
      return;
    }
    ...
    try {
      ...
      Logger.info(
        `[RevocationHandler] ✅ Message ${revokedMessageId} marked as REVOKED (by: ${fromMe ? "sender" : "contact"})`,
      );
      ...
      Logger.error(
        `[RevocationHandler] ❌ Failed to process revocation for ${revokedMessageId}:`,
        error,
      );
    }
  }
