...
export class FlowExecutorService {
   ...
   */
  private async getActiveSession(contactId: string): Promise<any | null> {
    return await prisma.contactFlowSession.findFirst({
      where: {
        contactId,
        isActive: true,
        isPaused: false,
  ...
  private async checkTriggers(
    ...
    for (const flow of flows) {
      const triggerData = flow.triggerData as any;
  ...
  private async handleSendNode(
    ...
    string | { type: string; url: string; message?: string; filename?: string }
  > {
  ...
  private async handleConditionNode(
    ...
    const valueToCheck = session.variables[variable] || userMessage;
