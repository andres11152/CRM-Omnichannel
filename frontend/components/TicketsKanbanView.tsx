...
export const TicketsKanbanView: React.FC<Props> = ({ isWidget = false }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-[#0b141a]">
      {/* Stats Toolbar */}
      <div className={`bg-white dark:bg-[#202c33] border-b border-gray-200 dark:border-gray-700 flex justify-between items-center ${isWidget ? 'px-4 py-2' : 'px-8 py-4'}`}>
        ...
        <div className="flex gap-2">
           <select 
             className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm px-3 py-1.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-gray-700 dark:text-gray-300"
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-6 h-full min-w-max">
            {Object.values(STATUS_COLUMNS).map((column) => (
              <div key={column.id} className="w-80 flex flex-col h-full bg-gray-100/50 dark:bg-[#111b21]/50 rounded-xl border border-gray-200 dark:border-gray-700/50">
                <div className={`p-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-[#202c33] rounded-t-xl sticky top-0 z-10 border-t-4 ${
                        ...
                        .map((ticket, index) => (
                          <Draggable key={ticket.id} draggableId={ticket.id} index={index}>
                            {(provided, snapshot) =>                                <div
                                  ...
                                  className={`bg-white dark:bg-[#2a3942] rounded-lg shadow-sm border mb-3 hover:shadow-md hover:-translate-y-1 transition-all duration-200 overflow-hidden group ${
      ...
      {showViewModal && selectedTicket && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleCloseModals}>
          <div className="bg-white dark:bg-[#2a3942] rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            {/* Modal Header */}
      ...
      {showAssignModal && selectedTicket && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleCloseModals}>
          <div className="bg-white dark:bg-[#2a3942] rounded-xl shadow-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            {/* Modal Header */}
                    ...
                    {agents.length === 0 ? (
                      ...
                      agents.map((agent: Agent) => (
                        <button
                          ...
                          className="w-full flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors border-2 border-transparent hover:border-indigo-500"
                        >
                          {agent.avatar ? (
            ...
            {/* Modal Footer */}
            <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex justify-end rounded-b-xl">
              <button 
                onClick={handleCloseModals}
                className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors"
              >
