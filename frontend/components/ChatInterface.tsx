...
export const ChatInterface: React.FC<Props> = ({
      ...
      try {
        ...
        if (data.status === "success" && data.data.conversation) {
          ...
          const history = conv.messages.map((m: any, index: number) => {
            ...
            // ”¥ CRITICAL FIX: Check metadata for AI-generated messages
  ...
  useEffect(() => {
    ...
    const joinRoom = () => {
      ...
      console.log(
        `[ChatInterface] šª Joining conversation room: ${activeContact.id}`,
      );
    ...
    const handleConnect = () => {
      console.log("[ChatInterface] Ÿ¢ Socket Connected");
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (now - lastRun < 50) {
        console.log(
          "[ChatInterface] ⏭️ Skipping duplicate reconciliation (< 50ms)",
        );
        return;
      }
      ...
      setMessages((prev) => {
        ...
        if (serverMsgExists) {
          ...
          if (!tempExists) {
            console.log("[API] ⏭️ Socket already handled, temp gone");
  ...
  const handleDataRequestSelect = async (type: string) => {
    setActionModalType(null);
    const text = `📋 *SOLICITUD DE DATOS*\n\nHola, para continuar con tu proceso, por favor confírmanos tu: *${type}*.\n\nPuedes responder a este mensaje.`;
    handleDirectSend(text);
  };
  ...
  const handleTypingIndicator = () => {
    // ⌨️ Emit typing event to backend for WhatsApp Sync
    const token = localStorage.getItem("token");
    ...
    <>
      <div className="flex h-full w-full overflow-hidden relative">
        ...
        <div className="flex flex-col flex-1 min-w-0 h-full relative">
          ...
          <div className="flex-shrink-0">
            <ChatHeaderEnhanced
              ...
              onForceReconnect={async () => {
                console.log("”„ Forcing socket + WhatsApp reconnection...");
                const token = localStorage.getItem("token");
                ...
                try {
                  toast.loading("Reiniciando conexión WhatsApp...");
                  ...
                  if (
                    ...
                    if (targetSessions.length > 0) {
                      ...
                      toast.success("Solicitud de reconexión enviada");
                    } else {
                      ...
                      if (sessions.length > 0) {
                        ...
                        toast.success("Reiniciando sesión principal...");
                      } else {
                        toast.info(
              ...
              responseTimeSLA={15}
              onAssignTo={(agentId) => {
                console.log("Ž¯ Asignando ticket a agente:", agentId);
          ...
          {/* Tag Menu (posició³n absoluta sobre el header) */}
          {showTagMenu && (
            <div className="absolute right-6 top-20 w-72 bg-white dark:bg-[#1f2937] rounded-xl shadow-2xl border-2 border-gray-200 dark:border-gray-700 p-3 z-[100] max-h-96 overflow-y-auto custom-scrollbar">
              ...
              {tags.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {tags.map((tag) => (
                    <label
                      key={tag.id}
                      className="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg cursor-pointer group transition-all border border-transparent hover:border-indigo-200 dark:hover:border-indigo-900"
                    >
                      <input
                        type="checkbox"
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden bg-gray-50 dark:bg-[#0b141a]">
            <div className="p-3 sm:p-4 space-y-2">
                  ...
                  >
                    ...
                    ${
                      isOutgoing
                        ...
                        : "bg-white dark:bg-reply-panel-dark text-reply-text dark:text-reply-text-dark rounded-tl-none border border-gray-100 dark:border-gray-700"
                    }
                  `}
                    >
                      {isBot && (
                      ...
                      {msg.content &&
                        ...
                        !/^📷 Imagen adjunta$/.test(msg.content) &&
                        msg.content !== "📍 Ubicación compartida" &&
                        !(
                          msg.content.startsWith("👤 Contacto:") &&
                          msg.attachment?.type === "contact"
                        ) &&
                        !(
                        ...
                        (() => {
                          ...
                          // Special Renderers
                          if (
                            content.startsWith("🕒 MENSAJE PROGRAMADO:") ||
                            ...
                            if (content.startsWith("🕒 MENSAJE PROGRAMADO:")) {
                              // Optimistic Format
                              const lines = content.split("\n\n");
                              realMsg = lines[1] || "";
                              dateDisplay =
                                lines[2]?.replace("📅 Para: ", "") || "";
                            } else {
                              // Persisted DB Format
                              realMsg = content;
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white dark:bg-reply-panel-dark px-4 py-2 rounded-xl rounded-tl-none shadow-sm border border-gray-100 dark:border-gray-700">
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 px-4 py-2 rounded-xl rounded-tl-none shadow-sm border border-blue-100 dark:border-blue-800">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">
                        ...
                        {otherAgentsTyping.length === 1 ? "estó¡" : "estó¡n"}{" "}
                        escribiendo...
                      </span>
                    </div>
                  </div>
          ...
          {readOnly ? (
            <div className="bg-gray-100 dark:bg-gray-800 px-4 py-3 border-t border-gray-200 dark:border-gray-700 text-center flex-shrink-0">
              <p className="text-sm text-gray-500 dark:text-gray-400 italic">
                ”’ El chat estó¡ en modo solo lectura.
              </p>
            </div>
          ) : (
      ...
      {showMediaLibrary && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-8">
          <div className="bg-white dark:bg-[#0b141a] w-full max-w-6xl h-[80vh] rounded-xl shadow-2xl overflow-hidden flex flex-col">
            <MediaLibrary
      ...
      {transferSuccess && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 animate-fade-in backdrop-blur-sm">
          <div className="bg-white dark:bg-[#202c33] rounded-2xl shadow-2xl max-w-sm w-full p-8 transform transition-all scale-100 border border-gray-100 dark:border-gray-700 text-center">
