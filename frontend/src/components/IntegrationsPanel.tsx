...
export const IntegrationsPanel: React.FC = () => {
  ...
  // Instagram DM configuration states
  ...
  const handleCreateInstagramSession = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading("Vinculando cuenta de Instagram...");
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_BASE_URL}/instagram/sessions`, {
        method: "POST",
      ...
      if (res.status === 201 && data.status === "success") {
        toast.success("Cuenta de Instagram vinculada exitosamente", { id: toastId });
        setIsConnectingInstagram(false);
        fetchInstagramSessions();
      } else {
        toast.error(data.message || "Error al vincular Instagram", { id: toastId });
      }
    } catch (err) {
      console.error(err);
      toast.error("Error de conexión al vincular Instagram", { id: toastId });
    } finally {
  ...
  const handleDeleteInstagramSession = async (id: string) => {
    try {
      ...
      toast.error("No se pudo desconectar la cuenta de Instagram");
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-reply-bg dark:bg-reply-bg-dark transition-colors duration-200">
      <ModuleHeader
        ...
        stats={{
          ...
          value:
            ...
            instagramSessions.filter((s) => s.status === "CONNECTED").length,
        }}
      />
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatTile
              label="WhatsApp"
              value={`${sessions.filter((s) => s.status === "CONNECTED").length}/${sessions.length}`}
            ...
            <StatTile
              label="Instagram"
              value={`${instagramSessions.filter((s) => s.status === "CONNECTED").length}/${instagramSessions.length}`}
          ...
          <section className="bg-white dark:bg-reply-panel-dark rounded-2xl border border-gray-200 dark:border-reply-border-dark shadow-sm p-6 md:p-8">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-9 h-9 rounded-lg bg-green-50 dark:bg-green-900/20 flex items-center justify-center text-green-600 dark:text-green-400 flex-shrink-0">
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
              </div>
              <div className="min-w-0">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Números conectados vía WhatsApp Web o Meta Cloud API.
                </p>
              </div>
            </div>
          ...
          <section className="bg-white dark:bg-reply-panel-dark rounded-2xl border border-gray-200 dark:border-reply-border-dark shadow-sm p-6 md:p-8">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400 text-[11px] font-bold border border-pink-200 dark:border-pink-800">
                    {instagramSessions.length} {instagramSessions.length === 1 ? "cuenta" : "cuentas"}
                  </span>
                </div>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Cuentas profesionales de Instagram conectadas vía Graph API.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              ...
              {/* Add New Instagram Account Card */}
              <button
              ...
              >
                ...
                <p className="text-sm text-gray-500 text-center mt-1">
                  Vincula una cuenta profesional de Instagram
                </p>
              </button>
            </div>
          </section>
          ...
          <section className="bg-reply-bg/60 dark:bg-reply-surface-dark/40 rounded-2xl border border-dashed border-gray-200 dark:border-reply-border-dark p-6 md:p-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5 mb-6">
              Nuevas integraciones en camino para centralizar aún más canales.
            </p>
      ...
      <Modal
        isOpen={isConnectingInstagram}
        onClose={() => setIsConnectingInstagram(false)}
        title="Conectar Instagram"
        subtitle="Vincula una cuenta profesional de Instagram vía la Graph API de Meta"
        size="md"
      >
        <form
          id="instagram-connect-form"
          onSubmit={handleCreateInstagramSession}
        ...
        >
          <div>
            ...
            <input
              ...
              placeholder="EAA..."
              className="w-full text-sm p-2.5 rounded-lg border border-gray-200 dark:border-reply-border-dark bg-reply-bg dark:bg-gray-800 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-pink-500 outline-none transition-all"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">
          ...
          <div>
            ...
            <input
              ...
              readOnly
              value={igVerifyToken}
              className="w-full text-sm p-2.5 rounded-lg border border-gray-200 dark:border-reply-border-dark bg-gray-100 dark:bg-gray-900 text-gray-500 dark:text-gray-400 font-mono"
            />
            <p className="text-xs text-gray-400 mt-1">
              Copia este token al configurar el webhook de Instagram en Meta for Developers.
            </p>
          </div>
        </form>
...
const StatTile = ({
  ...
  <div className="bg-white dark:bg-reply-panel-dark rounded-2xl border border-gray-200 dark:border-reply-border-dark shadow-sm p-5 flex items-center gap-4">
    <div
      className={`w-11 h-11 rounded-xl bg-gradient-to-br ${accent} flex items-center justify-center text-white shadow-sm flex-shrink-0`}
    >
      {icon}
    </div>
    <div className="min-w-0">
