...
export const TransferModal: React.FC<Props> = ({ isOpen, onClose, onTransfer }) => {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#202c33] w-full max-w-md rounded-xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <h3 className="text-lg font-bold text-gray-800 dark:text-white">Transferir Ticket</h3>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
          <button 
            ...
            onClick={() => { setActiveTab('AGENTS'); setSelectedId(null); }}
          >
            Agentes
          </button>
          <button 
            ...
            onClick={() => { setActiveTab('QUEUES'); setSelectedId(null); }}
          >
            Colas / Departamentos
          </button>
        </div>
        ...
        <div className="p-4">
          <input 
            type="text" 
            placeholder={activeTab === 'AGENTS' ? "Buscar agente..." : "Buscar cola..."}
            value={searchTerm}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            filteredAgents.length === 0 ? (
              <div className="text-center text-gray-400 py-4">No se encontraron agentes</div>
            ) : (
              filteredAgents.map(agent => (
                ...
                >
                  <div className="relative">
                    <img src={agent.avatar || `https://ui-avatars.com/api/?name=${agent.name}`} alt="" className="w-10 h-10 rounded-full" />
                    <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-[#202c33] ${['online', 'available'].includes(agent.status) ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-sm text-gray-800 dark:text-white">{agent.name}</h4>
            ...
            filteredQueues.length === 0 ? (
              <div className="text-center text-gray-400 py-4">No se encontraron colas</div>
            ) : (
              filteredQueues.map(queue => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-sm text-gray-800 dark:text-white">{queue.name}</h4>
                    <p className="text-xs text-gray-500">{queue.departmentDetails?.name || 'General'}</p>
                  </div>
                  {queue.aiAssistantId && (
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">
                      🤖 IA
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 font-medium text-sm">Cancelar</button>
          <button 
