...
const FEATURE_META: Record<string, { label: string; description: string; icon: React.ReactNode; type: 'number' | 'boolean' }> = {
  max_users: { 
    label: "Usuarios Máximos", 
    description: "Cantidad total de agentes permitidos.", 
    icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>, 
    type: 'number' 
  },
  max_queues: { 
    label: "Colas de Atención", 
    description: "Número de flujos de enrutamiento.", 
  ...
  max_whatsapp_connections: { 
    ...
    description: "Límmite de números de teléfono conectados.", 
    icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>, 
    type: 'number' 
  },
  can_use_ai: { 
    label: "Habilitar Motor IA", 
    description: "Acceso a bots inteligentes y respuestas automáticas.", 
  ...
  can_use_api: { 
    label: "API & Webhooks", 
    description: "Acceso para desarrolladores e integraciones.", 
  ...
  storage_limit_gb: {
    ...
    description: "Ej: 10 GB. Usa -1 para ilimitado.",
  ...
  max_companies: {
    ...
    description: "Número máximo de organizaciones permitidas.",
  ...
  max_workflows: {
    ...
    description: "Automatizaciones simultáneas permitidas.",
    icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
    type: 'number'
  },
};
...
export const PlanManagement: React.FC<Props> = ({ onNavigateToDashboard }) => {
  ...
  const handleDeletePlan = async () => {
    ...
    if (window.confirm(`¿Estás seguro de que deseas eliminar el plan "${formData.name}"? Esta acción no se puede deshacer.`)) {
      setIsDeleting(true);
      try {
      ...
      <div className="flex-1 p-6 overflow-hidden">
        {isLoading ? (
          ...
          <div className="h-full flex gap-6">
            ...
            <div className="w-80 flex flex-col bg-white dark:bg-[#202c33] rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden">
              <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#111b21]">
                <h3 className="font-bold text-gray-700 dark:text-gray-200">Planes Disponibles</h3>
              </div>
                  ...
                  <div 
                    ...
                    className={`p-4 rounded-xl cursor-pointer transition-all border relative group ${
                      selectedPlanId === p.id 
                        ...
                        : 'bg-white dark:bg-[#2a3942] border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-500'
                    }`}
                  >
                    <div className="flex justify-between items-start mb-1">
                       <span className={`font-bold text-lg ${selectedPlanId === p.id ? 'text-indigo-700 dark:text-indigo-300' : 'text-gray-800 dark:text-white'}`}>
                         {p.name}
                       </span>
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#111b21]">
                 <button 
            ...
            <div className="flex-1 flex flex-col bg-white dark:bg-[#202c33] rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden">
              {formData ? (
                <>
                  <div className="flex-1 overflow-y-auto p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                        <div className="md:col-span-5">
                            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 tracking-wider">Nombre del Plan</label>
                            <input 
                              type="text" 
                              value={formData.name} 
                              onChange={e => handleInputChange('name', e.target.value)} 
                              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 bg-gray-50 dark:bg-[#111b21] text-gray-900 dark:text-white font-bold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" 
                              placeholder="Ej: Pro Plan"
                            />
                        </div>
                        <div className="md:col-span-3">
                            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 tracking-wider">Precio (USD)</label>
                            <div className="relative">
                                ...
                                <input 
                                  ...
                                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg pl-8 pr-4 py-3 bg-gray-50 dark:bg-[#111b21] text-gray-900 dark:text-white font-mono font-bold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" 
                                />
                            </div>
                        </div>
                        <div className="md:col-span-4">
                            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 tracking-wider">Stripe Price ID</label>
                            <input 
                              type="text" 
                              value={formData.stripePriceId || ''} 
                              onChange={e => handleInputChange('stripePriceId', e.target.value)} 
                              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 bg-gray-50 dark:bg-[#111b21] text-gray-900 dark:text-white font-mono text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" 
                              placeholder="price_..."
                            />
                        </div>
                    </div>
                    ...
                    <div className="mb-6">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-gray-800 dark:text-white text-lg flex items-center gap-2">
                           Límites y Features
                           <span className="text-xs font-normal text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">{currentFeatures.length}</span>
                        </h3>
                        ...
                        {availableFeaturesToAdd.length > 0 && (
                          <div className="relative group z-10">
                            ...
                            <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-[#1a262d] rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right">
                                <div className="text-xs font-bold text-gray-400 uppercase px-2 py-1 mb-1">Disponibles</div>
                                {availableFeaturesToAdd.map(key => (
                                  <button 
                                    key={key} 
                                    onClick={() => addFeature(key)} 
                                    className="w-full flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-[#2a3942] rounded-lg text-left transition-colors"
                                  >
                                      <div className="text-gray-500 dark:text-gray-400">{FEATURE_META[key]?.icon}</div>
                                      <div>
                                        <p className="font-bold text-sm text-gray-800 dark:text-gray-200">{FEATURE_META[key]?.label}</p>
                                        <p className="text-xs text-gray-500 dark:text-gray-500 line-clamp-1">{FEATURE_META[key]?.description}</p>
                                      </div>
                                  </button>
                                ))}
                            </div>
                      ...
                      <div className="mb-8">
                         <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 border-b border-gray-100 dark:border-gray-700 pb-2">Configuración General</h3>
                         <div className="grid grid-cols-1 gap-4">
                            {currentFeatures.filter(k => !['storage_limit_gb', 'max_contacts', 'max_companies', 'max_workflows'].includes(k)).map((key) => {
                              const value = (formData.config as any)[key];
                              const meta = FEATURE_META[key];
                              const label = meta?.label || key;
                              const description = meta?.description || "Configuración personalizada";
                              const icon = meta?.icon || <span className="text-2xl">⚙️</span>;
                              const type = meta?.type || (typeof value === 'boolean' ? 'boolean' : 'number');

                              return (
                                <div key={key} className="group relative flex items-center gap-5 bg-gray-50 dark:bg-[#111b21] p-5 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                                  <div className="p-3 bg-white dark:bg-[#202c33] rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-300">
                                    {icon}
                                  </div>
                                  
                                  <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-gray-800 dark:text-gray-200 text-base mb-0.5">{label}</h4>
                                  ...
                                  <div className="w-48">
                                     {type === 'boolean' ? (
                                        ...
                                        <input 
                                          ...
                                          className="w-full bg-white dark:bg-[#202c33] border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-sm font-bold text-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" 
                                        />
                                      )}
                                  </div>
                                  ...
                                  <button 
                                    ...
                                    className="absolute -top-2 -right-2 bg-white dark:bg-[#2a3942] text-gray-400 hover:text-red-500 p-1 rounded-full shadow-md border border-gray-200 dark:border-gray-600 opacity-0 group-hover:opacity-100 transition-all transform hover:scale-110"
                      ...
                      <div>
                         <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 border-b border-gray-100 dark:border-gray-700 pb-2">Límites de Recursos (Quotas)</h3>
                         <div className="grid grid-cols-1 gap-4">
                            {['storage_limit_gb', 'max_contacts', 'max_companies', 'max_workflows'].map((key) => {
                                ...
                                if (!(formData.config as any).hasOwnProperty(key)) return null; 
                                ...
                                return (
                                    <div key={key} className="group relative flex items-center gap-5 bg-gray-50 dark:bg-[#111b21] p-5 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-bold text-gray-800 dark:text-gray-200 text-base mb-0.5 leading-tight">{meta?.label}</h4>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{meta?.description}</p>
                                            {key === 'storage_limit_gb' && <p className="text-[10px] text-indigo-500 font-medium mt-1">💡 1 GB ≈ 500 imágenes de alta calidad</p>}
                                        </div>
                                        ...
                                        <div className="w-56 flex flex-col items-end gap-2">
                                            ...
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <button 
                                                    type="button"
                                                    onClick={() => handleConfigChange(key, isUnlimited ? 0 : -1)} // Toggle: If unlim (-1) -> 0 (limit mode). If limit -> -1
                                                ...
                                                >
                                                    <div className={`w-3 h-3 bg-white rounded-full shadow-sm transform transition-transform duration-200 ease-in-out ${isUnlimited ? 'translate-x-4' : 'translate-x-0'}`} />
                                                </button>
                                            </div>
                                            ...
                                            <input 
                                                ...
                                                onChange={e => handleConfigChange(key, Number(e.target.value))} 
                                                className={`w-full bg-white dark:bg-[#202c33] border rounded-lg px-4 py-2 text-sm font-bold transition-all
                                                    ${isUnlimited 
                                                        ? 'border-gray-200 dark:border-gray-700 text-gray-400 italic cursor-not-allowed bg-gray-50 dark:bg-[#1a242b]' 
                                        ...
                                        <button 
                                            onClick={() => removeFeature(key)} 
                                            className="absolute -top-2 -right-2 bg-white dark:bg-[#2a3942] text-gray-400 hover:text-red-500 p-1 rounded-full shadow-md border border-gray-200 dark:border-gray-600 opacity-0 group-hover:opacity-100 transition-all transform hover:scale-110 z-10"
                                            title="Eliminar característica"
                                        >
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                        </button>
                  ...
                  {/* FOOTER ACTIONS */}
                  <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#111b21] flex justify-between items-center">
                      <button 
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                   <div className="w-24 h-24 bg-gray-100 dark:bg-[#111b21] rounded-full flex items-center justify-center mb-4">
                   ...
                   <h3 className="text-xl font-bold text-gray-600 dark:text-gray-300 mb-2">Ningún plan seleccionado</h3>
                   <p className="max-w-xs mx-auto">Selecciona un plan de la lista o crea uno nuevo para comenzar a editar.</p>
                </div>
              )}
            </div>
          </div>
