...
export const ChatHeaderEnhanced: React.FC<ChatHeaderEnhancedProps> = ({
  ...
  return (
    <div className="flex flex-col w-full z-30 transition-all duration-300">
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                {/* Tags — DESKTOP ONLY (Hidden on Mobile, moved to separate bar below) */}
                {!isTyping && (
                  <div className="hidden md:flex items-center gap-1.5 overflow-x-auto no-scrollbar w-full mask-linear-fade h-5">
                    {contact.tags && contact.tags.length > 0 ? (
                      ...
                      <span className="text-[10px] italic opacity-60 text-gray-500 dark:text-gray-400">
                        Sin etiquetas
                      </span>
                    )}
                  </div>
          ...
          {!isTightMode && (
            <div className="flex items-center gap-2 hidden lg:flex">
              ...
              <div className="relative">
                ...
                {showPriorityMenu && (
                  <div className="absolute right-0 top-full mt-2 w-32 bg-white dark:bg-reply-panel-dark rounded-lg shadow-xl border border-gray-200 dark:border-reply-border-dark overflow-hidden z-[100]">
                    {(["HIGH", "MEDIUM", "LOW"] as const).map((priority) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 text-[10px] font-bold hover:bg-reply-bg dark:hover:bg-gray-800 flex items-center gap-2"
                      >
                        {priority === "HIGH" && "??"}{" "}
                        {priority === "MEDIUM" && "??"}{" "}
                        {priority === "LOW" && "??"}
      ...
      {/* 📱 MOBILE TAG BAR: Dedicated scrollable row below main header */}
      <div className="md:hidden">
        {contact.tags && contact.tags.length > 0 && (
          <div className="bg-gray-50/95 dark:bg-gray-800/95 backdrop-blur-sm border-b border-gray-200 dark:border-reply-border-dark px-3 py-1 flex items-center gap-2 overflow-x-auto no-scrollbar scroll-smooth w-full shadow-inner">
            <div className="flex-shrink-0 text-[8px] font-bold text-gray-400 uppercase tracking-wider mr-1 select-none">
              Etiquetas
            </div>
            {contact.tags.map((tagId: string) => {
              const tag = availableTags.find(
              ...
              return (
                <span
                  key={tagId}
                  className={`px-2 py-0.5 rounded-full text-[9px] font-medium shadow-sm border border-black/5 whitespace-nowrap shrink-0 flex items-center gap-1 transition-transform active:scale-95 ${tag.color || "bg-white text-gray-700"}`}
                >
