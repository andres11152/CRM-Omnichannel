  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-64 gap-4">
        ...
        <p className="text-xs font-black text-gray-400 uppercase tracking-widest">{t("common.loading", "Cargando...")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-24 bg-white dark:bg-reply-panel-dark rounded-3xl border-2 border-dashed border-gray-200 dark:border-reply-border-dark flex flex-col items-center">
        ...
        <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-2">
          {t("team.table.empty_title", "Tu equipo está vacío")}
        </h3>
        <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-sm mx-auto">
          {t("team.table.empty_desc", "Comienza a agregar agentes para gestionar tus conversaciones de manera eficiente y profesional.")}
        </p>
        <button
          onClick={onCreateNew}
      ...
      <div className="hidden lg:block bg-white dark:bg-reply-panel-dark rounded-2xl border border-gray-100 dark:border-reply-border-dark shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#FBFCFE] dark:bg-reply-surface-dark border-b border-gray-100 dark:border-reply-border-dark">
            <tr>
              <th className="px-6 py-3.5 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">{t("team.table.agent", "Agente")}</th>
              <th className="px-6 py-3.5 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">{t("team.table.status", "Estado")}</th>
              <th className="px-6 py-3.5 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">{t("team.table.capacity", "Capacidad")}</th>
              <th className="px-6 py-3.5 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">{t("team.table.performance", "Rendimiento")}</th>
              <th className="px-6 py-3.5 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">{t("team.table.activity", "Actividad")}</th>
              <th className="px-6 py-3.5 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] text-right">{t("team.table.actions", "Acciones")}</th>
            </tr>
          </thead>
        ...
        {agents.map((agent) => (
          ...
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {agent.role === "Admin" ? (
                      <span className="px-2 py-0.5 rounded-lg text-[9px] font-black bg-purple-500/10 text-purple-600 dark:text-purple-400 uppercase tracking-widest">{t("team.roles.admin", "ADMIN")}</span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-lg text-[9px] font-black bg-gray-500/10 text-gray-500 dark:text-gray-400 uppercase tracking-widest">{t("team.roles.agent", "AGENT")}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-reply-border-dark">
              <div>
                <p className="text-[9px] text-gray-400 uppercase font-black tracking-widest mb-1">{t("team.table.workload", "Workload")}</p>
                <p className="text-sm font-black text-gray-900 dark:text-white">
                  {agent.currentLoad} / {agent.maxCapacity}
                </p>
              </div>
              <div>
                <p className="text-[9px] text-gray-400 uppercase font-black tracking-widest mb-1">{t("team.table.time_online", "Time Online")}</p>
...
const TeamTableRow: React.FC<TeamTableRowProps> = ({
  ...
  const statusConfig = {
    online: { color: "emerald", label: t("common.status.online", "Online") },
    busy: { color: "rose", label: t("common.status.busy", "Ocupado") },
    away: { color: "amber", label: t("common.status.away", "Ausente") },
    offline: { color: "gray", label: t("common.status.offline", "Offline") },
  };
  ...
  return (
    <tr className="hover:bg-blue-50/30 dark:hover:bg-indigo-500/5 transition-all group">
        ...
        <div className="w-40">
          <div className="flex justify-between items-end mb-2">
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
              {t("team.table.workload_current", "Carga Actual")}
            </span>
      ...
      <td className="px-6 py-2.5">
        <div className="flex items-center gap-6">
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("team.table.resolved", "Resueltos")}</span>
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("team.table.csat", "CSAT")}</span>
      ...
      <td className="px-6 py-2.5 text-right">
        {!agent.isAI && (
          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
            ...
            <button
              ...
              title={agent.isOwner ? t("team.table.owner_no_delete", "No se puede eliminar al Dueño") : t("common.delete", "Eliminar")}
            >
              <Trash2 size={18} />
            </button>
          </div>
        ...
        {agent.isAI && (
          <div className="text-[10px] font-black text-indigo-400 uppercase tracking-widest italic opacity-50">{t("common.saas_managed", "SaaS Managed")}</div>
        )}
      </td>
    </tr>
  );
