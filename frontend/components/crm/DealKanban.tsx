...
export const DealKanban: React.FC = () => {
  ...
  const createDefaultPipeline = async () => {
    try {
        ...
        if (msg?.includes('ya está en uso') || msg?.includes('already exists')) {
            console.log('[Pipeline] Default pipeline already exists, skipping creation');
  ...
  const fetchData = async () => {
    try {
      ...
      let defaultPipeline = pipelines.find((p: Pipeline) => p.isDefault) || pipelines[0];
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    setDeals(prev => prev.map(d => 
      d.id === draggableId ? { ...d, stageId: newStageId, order: newOrder } : d
    ));
          ...
          <div className="flex gap-4 h-full min-w-max">
            {pipeline?.stages?.sort((a, b) => a.order - b.order).map((stage) => {
              ...
              const getColorClasses = (hexColor: string) => {
                ...
                return colorMap[hexColor] || 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300';
              };

              return (
                <div key={stage.id} className="w-80 flex flex-col h-full bg-gray-100 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
                  <div className="p-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-reply-panel-dark rounded-t-xl sticky top-0 z-10">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="font-bold text-gray-700 dark:text-gray-200 text-sm uppercase">{stage.name}</h3>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${getColorClasses(stage.color)}`}>
                        {columnDeals.length}
                      </span>
                    </div>
                            ...
                            {(provided, snapshot) => (
                              <div
                                ...
                                className={`bg-white dark:bg-reply-panel-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-3 p-3 cursor-pointer hover:shadow-md transition-all group ${
                                  snapshot.isDragging ? 'rotate-2 scale-105 shadow-xl z-50 ring-2 ring-reply-blue' : ''
                                }`}
                                  ...
                                  <div className="flex items-center gap-2">
                                    {deal.probability > 0 && (
                                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                                        deal.probability >= 80 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                        deal.probability >= 50 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' :
